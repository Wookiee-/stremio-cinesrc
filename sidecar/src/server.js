...
import { parseStreamQuery, fetchProviderCatalog, resolveProviderStream, resolveProvidersBatch, resolveFirstStream } from './resolve/live.js'

const root = path.dirname(fileURLToPath(import.meta.url))
const publicDir = path.join(root, '../public')
const hlsPath = path.join(root, '../node_modules/hls.js/dist/hls.mjs')
const port = Number(process.env.SIDECAR_PORT || process.env.PORT) || 8001
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && pathOnly === '/api/stream/batch') {
      const params = query(req.url)
      const parsed = parseStreamQuery(params)
      if (parsed.error) return json(res, 400, { error: parsed.error })
      // ?providers=nebula,lisbon,surge — omit to probe all catalog providers.
      const ids = typeof params.providers === 'string' && params.providers.length
        ? params.providers.split(',').map((s) => s.trim()).filter(Boolean)
        : (params.provider ? [params.provider] : [])
      return apiJson(res, () => resolveProvidersBatch(parsed, ids))
    }
