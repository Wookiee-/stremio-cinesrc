...
function providerFields(provider, ms, stream) {
  return { ms, id: provider.id, name: provider.name, rank: provider.rank,
           flags: provider.flags || [], ...stream }
}
...
export async function resolveProvidersBatch(input, providerIds) {
  // One catalog + one action discovery, then one challenge host PER
  // provider probed in parallel. A single shared host would serialize all
  // mints via mintLock and hit per-provider timeouts, so parallel hosts
  // (same as N parallel /api/stream/provider calls) stay fastest. The win
  // vs N separate HTTP calls: 1 catalog fetch + 1 HTTP round-trip instead
  // of N. Wall time ~= slowest probe, not the sum.
  const { providers } = await fetchProviderCatalog(input)
  let wanted = providers
  if (Array.isArray(providerIds) && providerIds.length) {
    const set = new Set(providerIds)
    wanted = providers.filter((entry) => set.has(entry.id))
    if (!wanted.length) throw new Error(`Unknown providers: ${providerIds.join(',')}`)
  }
  const actions = await discoverServerActions(input.embedPath)
  const started = Date.now()
  const results = await Promise.all(
    wanted.map(async (provider) => {
      const t0 = Date.now()
      try {
        const result = await withChallengeHost(input, async (host) =>
          probeProvider(host, provider, input, actions),
        )
        const fields = providerFields(provider, Date.now() - t0, result)
        if (result.reason) return { ok: false, ...fields, reason: result.reason }
        return { ok: true, ...fields }
      } catch (error) {
        return {
          ok: false,
          ...providerFields(provider, Date.now() - t0, {}),
          reason: String(error?.message || error),
        }
      }
    }),
  )
  // Preserve rank order (catalogProviders already sorts desc).
  results.sort((a, b) => (b.rank ?? 0) - (a.rank ?? 0))
  return { ...meta(input), ms: Date.now() - started, providers: results }
}
